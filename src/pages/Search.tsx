...
import { Search as SearchIcon, ArrowLeft, ArrowUpLeft, SlidersHorizontal, ThumbsUp, CheckCircle2, Play, MessageSquare, Plus, GitBranch, Github, Clock, ChevronDown, Compass, User, X, TrendingUp, Hash } from "lucide-react";
import { collection, query, limit, getDocs, where, orderBy, doc, getDoc, setDoc, deleteDoc, writeBatch, increment, serverTimestamp } from "firebase/firestore";
...
import { syncSearchToRtdb, getHotSearchesAndTagsFromRtdb, getRecentSearchesFromRtdb } from "@/src/lib/rtdb-sync";
...
function SearchFollowButton({ targetId }: { targetId: string }) {
  ...
  useEffect(() => {
    ...
    const checkFollow = async () => {
      try {
        const followDoc = await getDoc(doc(db, "followers", `${user.uid}_${targetId}`));
        setIsFollowing(followDoc.exists());
      } catch (e: any) {
        if (e?.message?.includes("offline") || e?.code === "unavailable") {
          console.warn("Firestore client is offline. Search follow status loaded in offline mode.");
        } else {
...
function SearchLikeButton({ postId, authorId, initialLikesCount }: { postId: string, authorId?: string, initialLikesCount: number }) {
  const { user } = useAuth();
  ...
  return (
    <button 
      ...
      className={cn(
        ...
        hasLiked ? "text-buildops-blue" : "text-buildops-text-secondary hover:text-buildops-blue"
      )}
    >
      <motion.div 
         whileTap={{ scale: 0.8 }}
...
export function Search() {
  ...
  const lastRegisteredSearchRef = useRef<{ query: string, userId: string | null }>({ query: "", userId: null });
    ...
    const fetchHistory = async () => {
      try {
        ...
        try {
          ...
          console.warn("RTDB history fetch failed, falling back to Firestore:", rtdbErr);
        }

        // Fallback to Firestore if RTDB was empty/failed
        if (cloudHistory.length === 0) {
          const q = query(collection(db, "searchHistory"), where("userId", "==", user.uid));
        ...
        if (cloudHistory.length > 0) {
          setRecentSearches(prev => {
            ...
            const unique = Array.from(new Map(merged.map(item => [item.query.toLowerCase(), item])).values());
  ...
  useEffect(() => {
    if (searchQuery.trim()) {
      ...
      if (lastRegisteredSearchRef.current.query === term && lastRegisteredSearchRef.current.userId === (user?.uid || null)) {
         return;
      }
      lastRegisteredSearchRef.current = { query: term, userId: user?.uid || null };
      
      const newSearch = { id: term, query: term };
      setRecentSearches(prev => {
         const filtered = prev.filter(s => s.query.toLowerCase() !== term.toLowerCase());
  ...
  const unifiedSuggestions = useMemo(() => {
    ...
    // 1. History matches
    const historyMatches = recentSearches.filter(r => r.query.toLowerCase().includes(term));
    // Prioritize history that starts with the term
    historyMatches.filter(r => r.query.toLowerCase().startsWith(term)).forEach(r => addTextSuggestion(r.query, true));

    // 2. Exact query typed 
    if (!seen.has(term)) {
        list.splice(historyMatches.filter(r => r.query.toLowerCase().startsWith(term)).length, 0, { type: 'text', text: term, isHistory: false });
        seen.add(term);
    }
    ...
    suggestions.users.forEach(u => {
       if (u.id && !seenUsers.has(u.id)) {
           ...
           if (dn.startsWith(term) || un.startsWith(term) || dn.split(' ').some((w) => w.startsWith(term))) {
               seenUsers.add(u.id);
  ...
  useEffect(() => {
    ...
    const unsubscribeSearches = onValue(searchCountsRef, (snapshot) => {
      ...
      const fallbackQueries = ["builds", "software", "deploy", "engineering", "startup", "projects", "bugs", "solutions"];
    ...
    const unsubscribeTags = onValue(tagCountsRef, (snapshot) => {
      const data = snapshot.val();
      ...
      if (data) {
        ...
        const formattedTags = sortedTags.map(tag => {
          ...
          if (t === 'ai' || t === 'iot' || t === 'it' || t === 'api' || t === 'ui' || t === 'ux') return t.toUpperCase();
          return t.split(/[_-]/).map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
        });
        rawTags.push(...formattedTags);
      }

      const fallbackTags = ["Software", "Build", "Deployment", "Database", "AI", "Cloud", "Web", "Design", "Testing", "Security"];
  ...
  const [searchResults, setSearchResults] = useState({ problems: [] as any[], builds: [] as any[], solutions: [] as any[], builders: [] as any[] });
  const [loading, setLoading] = useState(!!searchQuery);
  const [lastSearchedQuery, setLastSearchedQuery] = useState("");

  if (searchQuery !== lastSearchedQuery) {
     setLastSearchedQuery(searchQuery);
     if (searchQuery.trim() || selectedCategories.length > 0 || selectedStatuses.length > 0 || selectedLevels.length > 0) {
       setLoading(true);
     }
  }
  ...
  useEffect(() => {
    if (!searchQuery.trim() && selectedCategories.length === 0 && selectedStatuses.length === 0 && selectedLevels.length === 0) {
      setSearchResults({ problems: [], builds: [], solutions: [], builders: [] });
      return;
    }
    const fetchSearch = async () => {
      ...
      try {
        ...
        if (lowerQ) {
            ...
            const res = await fetch(`/api/search?q=${encodeURIComponent(lowerQ)}`);
            if (res.ok) {
        ...
        if (!lowerQ && (dbPosts.length === 0 && dbComments.length === 0 && dbUsers.length === 0)) {
            const postsRef = collection(db, "posts");
            const q = query(postsRef, orderBy("createdAt", "desc"), limit(50));

            const snap = await getDocs(q);
            dbPosts = snap.docs.map(d => ({id: d.id, ...(d.data() as object)})).filter((p: any) => p.status !== 'trashed') as any[];
            
            const usersRef = collection(db, "users");
            const uQ = query(usersRef, limit(20));
            const uSnap = await getDocs(uQ);
            dbUsers = uSnap.docs.map(d => ({id: d.id, ...(d.data() as object)})) as any[];
        }
        ...
        const matchedPosts = allPosts.filter(p => {
           const matchesCategory = selectedCategories.length === 0 || selectedCategories.map(c => c.toLowerCase()).includes(p.category?.toLowerCase() || '');
           const matchesLevel = selectedLevels.length === 0 || selectedLevels.map(l => l.toLowerCase()).includes(p.level?.toLowerCase() || '');
           const matchesStatus = selectedStatuses.length === 0 || selectedStatuses.map(s => s.toLowerCase()).includes(p.status?.toLowerCase() || '');

           if (!(matchesCategory && matchesLevel && matchesStatus)) {
             return false;
           }

           const authorProfile = usersMap.get(p.authorId);
           return authorProfile && authorProfile.email && authorProfile.email.trim() !== "";
        });

        const matchedBuilders = allBuilders.filter(b => {
           const matchesSearch = !lowerQ || b.displayName?.toLowerCase()?.includes(lowerQ) || b.handle?.toLowerCase()?.includes(lowerQ) || (Array.isArray(b.interests) && b.interests.some((t: string) => typeof t === 'string' && t.toLowerCase().includes(lowerQ)));
           if (!matchesSearch) return false;

           const authorProfile = usersMap.get(b.id);
           return authorProfile && authorProfile.email && authorProfile.email.trim() !== "";
        });
        ...
        const finalResults = {
            ...
            solutions: matchedPosts.filter(p => p.type === 'solution' || p.type === 'comment'),
    ...
    fetchSearch();
  }, [searchQuery, selectedCategories, selectedStatuses, selectedLevels, sortBy]);

  const hasResults = searchResults.problems.length > 0 || searchResults.builds.length > 0 || searchResults.solutions.length > 0 || searchResults.builders.length > 0;
  ...
  const Filters = () => (
    <div className="space-y-6">
      {availableCategories.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold text-buildops-text-secondary uppercase tracking-wider mb-3">Category</h3>
          <div className="flex flex-col gap-2">
            {availableCategories.map(t => (
              <label key={t} className="flex items-center gap-2 text-sm text-buildops-text-secondary hover:text-buildops-text cursor-pointer">
                <input 
                   type="checkbox" 
                   ...
                   onChange={(e) => {
                      if (e.target.checked) setSelectedCategories([...selectedCategories, t]);
                      else setSelectedCategories(selectedCategories.filter(c => c !== t));
                   }}
                />
                {t}
              </label>
      ...
      {availableLevels.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold text-buildops-text-secondary uppercase tracking-wider mb-3">Level</h3>
          <div className="flex flex-col gap-2">
            {availableLevels.map(t => (
              <label key={t} className="flex items-center gap-2 text-sm text-buildops-text-secondary hover:text-buildops-text cursor-pointer">
                <input 
                  ...
                  onChange={(e) => {
                     if (e.target.checked) setSelectedLevels([...selectedLevels, t]);
                     else setSelectedLevels(selectedLevels.filter(c => c !== t));
                  }}
                />
                {t}
              </label>
      ...
      {availableStatuses.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold text-buildops-text-secondary uppercase tracking-wider mb-3">Status</h3>
          <div className="flex flex-col gap-2">
            {availableStatuses.map(t => (
              <label key={t} className="flex items-center gap-2 text-sm text-buildops-text-secondary hover:text-buildops-text cursor-pointer">
                <input 
                   ...
                   onChange={(e) => {
                      if (e.target.checked) setSelectedStatuses([...selectedStatuses, t]);
                      else setSelectedStatuses(selectedStatuses.filter(c => c !== t));
                   }}
                />
                {t}
              </label>
  ...
  return (
    <div className="flex flex-col min-h-screen pb-20 md:pb-0">
      ...
      <div className={cn(
        ...
        <button onClick={() => navigate(-1)} className="p-1 -ml-1 text-buildops-text-secondary hover:text-buildops-text shrink-0">
         ...
         <div className="fixed top-14 left-0 right-0 bottom-0 bg-buildops-bg z-50 overflow-y-auto md:hidden w-full h-[calc(100vh-56px)] pb-20">
             {unifiedSuggestions.length === 0 ? (
                <div className="flex items-center gap-4 px-4 py-3 border-b border-buildops-border/30">
                   ...
                   <span className="flex-1 text-buildops-text text-[15px]">{searchInput}</span>
                </div>
             ) : (
                <div className="flex flex-col py-2">
                   {unifiedSuggestions.map((sug, idx) => {
                     if (sug.type === 'user') {
                       const u = sug.user;
                       return (
                         <div key={`m-sug-user-${idx}`} onClick={() => { navigate(`/profile/${u.id}`); }} className="flex items-center gap-3 px-4 py-3 active:bg-buildops-card transition-colors cursor-pointer group text-buildops-text">
                            ...
                            <div className="flex flex-col flex-1 min-w-0">
                               <span className="text-buildops-text text-[15px] line-clamp-1 font-medium break-all">{u.displayName}</span>
                               {(u.username || u.handle) && <span className="text-sm text-buildops-text-secondary line-clamp-1 break-all">@{u.username || u.handle}</span>}
                            </div>
                            <button onClick={(e) => { e.stopPropagation(); setSearchInput(u.displayName || u.username || u.handle || ''); }} className="p-2 -mr-2 text-buildops-text-secondary">
                     ...
                     return (
                        <div key={`m-sug-${idx}`} onClick={() => { navigate(`/search?q=${encodeURIComponent(sug.text)}`); }} className="flex items-center gap-4 px-4 py-3 active:bg-buildops-card transition-colors cursor-pointer group">
                           ...
                           <button onClick={(e) => { e.stopPropagation(); setSearchInput(sug.text); }} className="p-2 -mr-2 text-buildops-text-secondary">
                             <ArrowUpLeft className="w-5 h-5 shrink-0" />
                           </button>
                        </div>
                     );
      ...
      {!searchQuery ? (
        ...
        <div className="flex-1 flex flex-col items-center pt-[2px] md:pt-16 max-w-3xl mx-auto w-full px-4">
          <h1 className="text-2xl md:text-3xl font-bold text-buildops-text mb-8 hidden md:block">Search engineering posts</h1>
          
          <form onSubmit={handleSearch} className="w-full relative mb-[2px] hidden md:block">
                   ...
                   <>
                       {unifiedSuggestions.map((sug, idx) => {
                         if (sug.type === 'user') {
                           const u = sug.user;
                           return (
                             <div key={`d-sug-user-${idx}`} onClick={() => { navigate(`/profile/${u.id}`); }} className="flex items-center gap-3 px-4 py-2 hover:bg-buildops-bg transition-colors cursor-pointer group text-buildops-text">
                                ...
                                <span className="flex-1 text-[15px] font-medium line-clamp-1 break-all">{u.displayName}</span>
                                {(u.username || u.handle) && <span className="text-sm text-buildops-text-secondary line-clamp-1 mr-2 opacity-50 group-hover:opacity-100 transition-opacity">@{u.username || u.handle}</span>}
                                <button onClick={(e) => { e.stopPropagation(); setSearchInput(u.displayName || u.username || u.handle || ''); }} className="p-1 -mr-1 text-buildops-text-secondary opacity-0 group-hover:opacity-100 transition-opacity">
                         ...
                         return (
                            <div key={`d-sug-${idx}`} onClick={() => { navigate(`/search?q=${encodeURIComponent(sug.text)}`); }} className="flex items-center gap-4 px-4 py-2 hover:bg-buildops-bg transition-colors cursor-pointer group text-buildops-text">
                               {sug.isHistory ? (
                                  <Clock className="w-4 h-4 text-buildops-text-secondary shrink-0" />
                               ) : (
                                  <SearchIcon className="w-4 h-4 text-buildops-text-secondary shrink-0" />
                               )}
                               <span className="flex-1 text-[15px] font-medium line-clamp-1 break-all">{sug.text}</span>
                               <button onClick={(e) => { e.stopPropagation(); setSearchInput(sug.text); }} className="p-1 -mr-1 text-buildops-text-secondary opacity-0 group-hover:opacity-100 transition-opacity">
                                  <ArrowUpLeft className="w-4 h-4 shrink-0" />
                               </button>
                            </div>
                         );
            ...
            {!historyLoaded ? (
              <div className="mb-6">
                 <div className="flex flex-col gap-1 -ml-3">
                    {[1, 2, 3, 4].map(i => (
                      <div key={i} className="px-3 py-2 rounded-lg text-left text-sm text-buildops-text flex items-center justify-between animate-pulse w-full">
                    ...
                    {recentSearches.map(search => (
                      ...
                      >
                             ...
                             <div className="absolute top-0 left-0 w-full h-full bg-buildops-bg/40 backdrop-blur-[2px] z-10 flex items-center justify-center rounded-lg">
                                <div className="bg-buildops-card border border-buildops-border p-2 rounded-lg shadow-lg flex flex-col items-center gap-2 max-w-[200px]">
                                    ...
                                    <div className="flex gap-2">
                                       <button onClick={(e) => { e.stopPropagation(); setLongPressedSearchId(null); }} className="px-3 py-1.5 text-xs font-medium text-buildops-text-secondary hover:bg-white/5 rounded-lg transition-colors">Cancel</button>
                                       <button onClick={(e) => { 
                                          e.stopPropagation();
                                          ...
                                          if (user && search.id !== search.query) {
                                             deleteDoc(doc(db, "searchHistory", search.id)).catch(console.error);
                                          }
                                       }} className="px-3 py-1.5 text-xs font-medium text-red-500 hover:bg-red-500/10 rounded-lg transition-colors">Delete</button>
                                    </div>
                                </div>
                             </div>
                         )}
                        <div
                          ...
                          onTouchStart={(e) => handleSearchPressStart(search.id)}
                        ...
                        >
                           <span className="flex items-start gap-3 flex-1 overflow-hidden">
                             <Clock className="w-4 h-4 shrink-0 text-buildops-text-secondary mt-[3px]" />
                             <span className="line-clamp-2 leading-tight break-words">{search.query}</span>
                           </span>
                           <button 
                             onClick={(e) => { 
                               e.stopPropagation(); 
            ...
            <div className="flex-1 w-full min-w-0">
               ...
               <div className="hidden md:flex items-center justify-between mb-4 pb-4 border-b border-buildops-border">
                  <div className="text-sm text-buildops-text-secondary">
                     Showing results for <span className="text-buildops-text font-medium">"{searchQuery}"</span>
                  </div>
               ...
               {loading ? (
                 ...
                 <div className="space-y-8">
                    {/* People Section */}
                    {(activeTab === "All" || activeTab === "Users") && searchResults.builders.length > 0 && (
                       <div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                             {searchResults.builders.map(b => (
                                <div key={b.id} className="p-4 rounded-xl border border-buildops-border bg-buildops-card shadow-sm hover:border-buildops-text-secondary/50 hover:shadow-md transition-all flex flex-col items-center text-center gap-2 group cursor-pointer" onClick={() => navigate(`/profile/${b.id}`)}>
                                   <div className="w-16 h-16 rounded-full border-2 border-buildops-bg shadow-sm overflow-hidden bg-buildops-card flex items-center justify-center shrink-0 mt-2">
                                      {b.photoURL ? (
                                        <img src={b.photoURL} alt={b.displayName} className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                                      ) : (
                                   ...
                                   <div>
                                      ...
                                      <p className="text-xs text-buildops-text-secondary mb-1 truncate">@{b.handle || 'user'}</p>
                                   </div>
                    ...
                    <div className="space-y-6">
                       {/* Post Result */}
                       {(activeTab === "All" || activeTab === "Posts") && searchResults.problems.map(p => (
                      <div key={p.id} className="py-4 px-3 sm:px-4 md:px-5 rounded-lg border border-buildops-border bg-buildops-card hover:border-buildops-text-secondary/50 transition-colors group flex flex-col gap-3">
                         <div className="flex flex-wrap items-center gap-2">
                           <span className="px-2 py-0.5 rounded text-[11px] font-mono font-medium border border-buildops-border bg-buildops-bg text-buildops-text-secondary">Post</span>
                           {p.category && <span className="px-2 py-0.5 rounded text-[11px] font-mono font-medium border border-buildops-border bg-buildops-bg text-buildops-text-secondary">{p.category}</span>}
                           {p.status && p.status !== "none" && p.status !== "open" && <span className="px-2 py-0.5 rounded text-[11px] font-mono font-medium border border-buildops-green/30 bg-buildops-bg text-buildops-green">{p.status}</span>}
                         </div>
                         
                         <div>
                           <h3 className="text-lg font-bold text-buildops-text mb-1 group-hover:text-blue-400 transition-colors cursor-pointer" onClick={() => navigate(`/problems/${p.id}`)}>
                           ...
                           <p className="text-sm text-buildops-text-secondary leading-relaxed line-clamp-2 break-words">
                              {renderTextWithMentions(p.body, settings.markdownRendering)}
                           </p>
                         </div>

                         <div className="flex items-center gap-2 text-sm text-buildops-text-secondary">
                            Tags: {p.tags?.map((t: string, i: number) => <span key={`${t}-${i}`} className="font-mono text-xs">· {t}</span>)}
                         </div>

                         <div className="flex flex-wrap items-center gap-4 text-sm font-medium mt-2">
                            <span className="text-buildops-text-secondary">{formatCount(p.stats?.commentsCount || 0)} Comments</span>
                            
                            <div className="ml-auto">
                                <SearchLikeButton postId={p.id} authorId={p.authorId} initialLikesCount={p.stats?.likesCount || 0} />
                             </div>
                         </div>
                         
                         <Link to={`/problems/${p.id}`} className="mt-2 text-sm font-medium text-buildops-blue hover:text-blue-400 transition-colors flex items-center gap-1">
                            View Post →
                         </Link>
                      </div>
                    ))}
                    ...
                    {/* Build Result */}
                     {(activeTab === "All" || activeTab === "Builds") && searchResults.builds.map(b => (
                        <div key={b.id} className="py-4 px-3 sm:px-4 md:px-5 rounded-lg border border-buildops-border bg-buildops-card hover:border-buildops-text-secondary/50 transition-colors group flex flex-col gap-3">
                           <div className="flex flex-wrap items-center gap-2">
                             <span className="px-2 py-0.5 rounded text-[11px] font-mono font-medium border border-buildops-border bg-buildops-bg text-buildops-text-secondary">Build</span>
                             {b.category && <span className="px-2 py-0.5 rounded text-[11px] font-mono font-medium border border-buildops-border bg-buildops-bg text-buildops-text-secondary">{b.category}</span>}
                             {b.status && b.status !== "none" && b.status !== "open" && <span className="px-2 py-0.5 rounded text-[11px] font-mono font-medium border border-buildops-green/30 bg-buildops-bg text-buildops-green">{b.status}</span>}
                           </div>
                           
                           <div>
                             <h3 className="text-lg font-bold text-buildops-text mb-1 group-hover:text-blue-400 transition-colors cursor-pointer" onClick={() => navigate(`/problems/${b.id}`)}>
                               {renderTextWithMentions(b.title, false)}
                             </h3>
                             <p className="text-sm text-buildops-text-secondary leading-relaxed line-clamp-2 break-words">
                                {renderTextWithMentions(b.body, settings.markdownRendering)}
                             </p>
                           </div>

                           <div className="flex items-center gap-2 text-sm text-buildops-text-secondary">
                              Tags: {b.tags?.map((t: string, i: number) => <span key={`${t}-${i}`} className="font-mono text-xs">· {t}</span>)}
                           </div>

                           <div className="flex flex-wrap items-center gap-4 text-sm font-medium mt-2">
                              <span className="text-buildops-text-secondary">{formatCount(b.stats?.commentsCount || 0)} Comments</span>
                              
                              <div className="ml-auto">
                                 <SearchLikeButton postId={b.id} authorId={b.authorId} initialLikesCount={b.stats?.likesCount || 0} />
                              </div>
                           </div>
                           
                           <Link to={`/problems/${b.id}`} className="mt-2 text-sm font-medium text-buildops-blue hover:text-blue-400 transition-colors flex items-center gap-1">
                              View Build →
                           </Link>
                        </div>
                     ))}
                     ...
                     {/* Comment Result */}
                    {(activeTab === "All" || activeTab === "Comments") && searchResults.solutions.map(s => (
                       <div key={s.id} className="py-4 px-3 sm:px-4 md:px-5 rounded-lg border border-buildops-border bg-buildops-bg hover:border-buildops-text-secondary/50 transition-colors group flex flex-col gap-3">
                          <div className="flex flex-wrap items-center gap-2">
                             <span className="px-2 py-0.5 rounded text-[11px] font-mono font-medium border border-buildops-border bg-buildops-card text-buildops-text-secondary">Comment</span>
                          </div>

                          <div>
                             <h3 className="text-lg font-bold text-buildops-text mb-2 group-hover:text-buildops-blue transition-colors cursor-pointer" onClick={() => navigate(`/problems/${s.postId}/submit`)}>
                               {s.title || (s.body ? s.body.substring(0, 30) + '...' : 'Untitled Comment')}
                             </h3>
                          </div>

                          <div className="bg-buildops-card p-3 rounded border border-buildops-border mt-1">
                             <Link to={`/problems/${s.postId}`} className="text-sm font-medium text-buildops-text hover:underline decoration-buildops-border underline-offset-2">
                                View original post
                             </Link>
                          </div>
                       </div>
                 ...
                 <div className="py-12 flex flex-col items-center text-center max-w-md mx-auto">
                    ...
                    <h3 className="text-xl font-bold text-buildops-text mb-2">No matching engineering posts found.</h3>
               ...
               <div className="mt-12 p-6 rounded-xl border border-dashed border-buildops-border/80 bg-buildops-card/30 flex flex-col items-center text-center transition-colors hover:border-buildops-border hover:bg-buildops-card/50">
                  ...
                  <h3 className="text-base font-semibold text-buildops-text mb-1">Can't find the post?</h3>
                  <p className="text-sm text-buildops-text-secondary mb-4">Post it and let builders solve it.</p>
                  <Link
                    to="/problems/new"
      ...
      {showMobileFilters && (
        <>
          <div className="fixed inset-0 bg-black/60 z-[60] transition-opacity" onClick={() => setShowMobileFilters(false)} />
          <div className="fixed inset-x-0 bottom-0 z-[60] bg-buildops-card border-t border-buildops-border rounded-t-2xl max-h-[85vh] flex flex-col pb-safe shadow-2xl">
            <div className="sticky top-0 bg-buildops-card border-b border-buildops-border px-4 py-3 flex items-center justify-between z-10 shrink-0 rounded-t-2xl">
              <button onClick={() => {
                  ...
                  setSelectedStatuses([]);
                  setSelectedLevels([]);
              }} className="text-sm font-medium text-buildops-text-secondary px-2 py-1 hover:text-buildops-text">Reset</button>
              <h2 className="text-base font-bold text-buildops-text">Filters</h2>
              <button onClick={() => setShowMobileFilters(false)} className="text-sm font-bold text-buildops-blue px-4 py-1.5 bg-buildops-blue/10 rounded-full hover:bg-buildops-blue/20 transition-colors">Apply</button>
            </div>
