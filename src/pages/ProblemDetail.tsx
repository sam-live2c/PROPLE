...
export function ProblemDetail() {
  ...
  const [post, setPost] = useState<any>(() =>
    sessionCache.get("post_" + id, null),
  );
  const [author, setAuthor] = useState<any>(() =>
    sessionCache.get("author_" + id, null),
  );
  const [isFollowing, setIsFollowing] = useState(false);
  const [comments, setComments] = useState<any[]>(() =>
    sessionCache.get("comments_" + id, []),
  );
  const { user, userProfile } = useAuth();

  const [loadingPost, setLoadingPost] = useState(
    () => !sessionCache.has("post_" + id),
  );
      ...
      getDoc(followRef)
        ...
        .catch((err) => {
          if (
            ...
            console.warn(
              "Firestore client is offline. Follow status loaded in offline mode.",
            );
          } else {
  ...
  useEffect(() => {
    ...
    const unsubSent = onSnapshot(
      ...
      (err) => {
        console.warn(
          "Offline or failed fetching sent notifications status:",
          err,
        );
      },
    );
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        setDoc(
          shareRef,
          { postId: id, userId: user.uid, createdAt: serverTimestamp() },
          { merge: true },
        ).catch(() => {});
      }
      if (navigator.share) {
  ...
  const handleToggleSave = async () => {
    ...
    setPost((prev: any) => ({
      ...
      stats: {
        ...
        savesCount: Math.max(
          0,
          (prev.stats?.savesCount || 0) + (newHasSaved ? 1 : -1),
        ),
      },
    }));
    ...
    const toastId = toast.success(
      newHasSaved ? "Post saved to bookmarks" : "Post removed from bookmarks",
    );
  ...
  const [expandedTextState, setExpandedTextState] = useState<
  ...
  const childrenMap = useMemo(() => {
    ...
    const extractSolutions = (currentId: string, depth: number): any[] => {
      ...
      for (const child of children) {
        foundSolutions = foundSolutions.concat(
          extractSolutions(child.id, depth + 1),
        );
      }

      const newChildren = children.filter((child) => {
        const checkIsSol =
          post?.solutionCommentIds?.includes(child.id) || child.isSolution;
  ...
  const isCommentInSolutionPath = useCallback(
    (cId: string): boolean => {
  ...
  useEffect(() => {
    if (id && user && post) {
      getDoc(doc(db, "likes", `${id}_${user.uid}`))
        .then((snap) => {
        ...
        .catch((err) => {
          if (
            ...
            console.warn(
              "Firestore client is offline. Like status loaded in offline mode.",
            );
          } else {
      ...
      getDoc(doc(db, "dislikes", `${id}_${user.uid}`))
        .then((snap) => {
        ...
        .catch((err) => {
          if (
            ...
            console.warn(
              "Firestore client is offline. Dislike status loaded in offline mode.",
            );
          } else {
      ...
      getDoc(doc(db, "saves", `${id}_${user.uid}`))
        .then((snap) => {
        ...
        .catch((err) => {
          if (
            ...
            console.warn(
              "Firestore client is offline. Save status loaded in offline mode.",
            );
          } else {
  ...
  const handleVoteComment = async (commentId: string, voteType: 1 | -1 | 0) => {
    ...
    try {
      ...
      if (targetComment?.authorId && targetComment.authorId !== user.uid) {
        batch.set(
          doc(db, "users", targetComment.authorId),
          {
      ...
      if (targetComment?.isSolution && post) {
        ...
        const currentDownvotes =
          (targetComment.downvotes || 0) + downvotesChange;
        ...
        if (currentDownvotes + currentReports >= threshold) {
          ...
          toast.info(
            "This solution was automatically unmarked due to negative feedback.",
          );
        }
      }
  ...
  const handleReportCommentSubmit = async (
    ...
    if (!user || user.isAnonymous) {
      toast.error("Guest users cannot report comments.");
      return;
    }
    try {
        ...
        prev.map((c) =>
          c.id === commentId
            ? {
                ...
                isSolution: wasSolutionRemoved ? false : c.isSolution,
              }
            : c,
        ),
      );
              ...
              setComments((prev) =>
                prev.map((c) =>
                  c.id === commentId
                    ? {
                        ...c,
                        reportsCount: Math.max(0, (c.reportsCount || 0) - 1),
                      }
                    : c,
                ),
              );
  ...
  const handleSendToTrash = async () => {
    ...
    try {
      await setDoc(
        doc(db, "posts", id),
        { status: "trashed", updatedAt: serverTimestamp() },
        { merge: true },
      );
        ...
        action: {
          ...
          onClick: async () => {
            try {
              await setDoc(
                doc(db, "posts", id),
                { status: originalStatus, updatedAt: serverTimestamp() },
                { merge: true },
              );
  ...
  if (
    !post ||
    (post.status === "trashed" && (!user || post.authorId !== user.uid))
  ) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center">
        ...
        <p className="text-buildops-text-secondary">
          This post doesn't exist, has been deleted, or you don't have access.
        </p>
        <Link
          to="/explore"
          className="mt-6 px-6 py-2.5 bg-buildops-text text-buildops-bg rounded-xl font-medium hover:bg-white transition-colors"
        >
          Go back to Feed
        </Link>
      </div>
  ...
  const handleLikeClick = async (e: React.MouseEvent) => {
    ...
    setPost((prev: any) => ({
      ...
      stats: {
        ...
        likesCount: Math.max(
          0,
          (prev.stats?.likesCount || 0) + (newHasLiked ? 1 : -1),
        ),
        dislikesCount: Math.max(
          0,
          (prev.stats?.dislikesCount || 0) + (shouldRemoveDislike ? -1 : 0),
        ),
      },
    }));
  ...
  const handleDislikeClick = async (e: React.MouseEvent) => {
    ...
    setPost((prev: any) => ({
      ...
      stats: {
        ...
        dislikesCount: Math.max(
          0,
          (prev.stats?.dislikesCount || 0) + (newHasDisliked ? 1 : -1),
        ),
        likesCount: Math.max(
          0,
          (prev.stats?.likesCount || 0) + (shouldRemoveLike ? -1 : 0),
        ),
      },
    }));
    ...
    <div className="flex flex-col lg:flex-row gap-8 pb-16 lg:pb-0 relative">
      ...
      <div className="flex-1 w-full max-w-4xl min-w-0">
        <div className="sticky top-0 z-40 bg-buildops-bg/95 backdrop-blur-md h-14 mb-0 px-3 sm:px-4 md:px-5 lg:px-6 border-b border-[rgba(255,255,255,0.05)] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-mono font-bold text-buildops-text">
              post
            </h1>
          </div>
          <div className="flex items-center gap-2 text-buildops-text-secondary">
            <Link
              to="/search"
              className="p-2 rounded-full text-buildops-text-secondary hover:text-buildops-text hover:bg-white/5 transition-colors flex items-center justify-center"
            >
                ...
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-48 bg-buildops-card border border-buildops-border rounded-xl shadow-xl z-50 py-1.5 text-left font-sans animate-fade-in">
                    ...
                    <button
                      ...
                      className="w-full text-left px-4 py-2.5 text-sm text-buildops-text hover:bg-white/5 transition-colors flex items-center gap-2.5 cursor-pointer border-0 bg-transparent font-medium"
                    >
                      <Forward className="w-4 h-4 text-buildops-text-secondary -rotate-[15deg]" strokeWidth={2.8} />
                      Share Post
                    </button>
                    <button
                      onClick={(e) => {
            ...
            <div className="flex flex-col justify-center">
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {author?.role === "verified" && (
                  <CheckCircle2 className="w-4 h-4 text-buildops-blue shrink-0" />
                )}
              </div>
              <Link
          ...
          {post?.authorId && user?.uid !== post.authorId && (
            <div className="shrink-0">
              <FollowButton
                ...
                variant="button"
                className="text-xs md:text-sm font-bold shadow-sm"
              />
            </div>
          )}
        </div>
        ...
        <div>
          <div className="pt-0 pb-6 px-3 sm:px-4 md:px-5 lg:px-6 border-b border-buildops-border bg-buildops-bg transition-colors">
            <div className="min-w-0">
                            ...
                            return (
                              <span>
                                ...
                                <button
                                  ...
                                  className="text-buildops-blue font-semibold hover:underline ml-1 inline-block bg-transparent border-0 p-0 cursor-pointer text-[14px] sm:text-[15px]"
                                >
                                  ...Read more
                                </button>
                              </span>
              ...
              {/* Action Buttons */}
              <div className="flex items-center justify-start gap-8 text-buildops-text-secondary mt-2 pt-2 border-t border-buildops-border/30">
                {/* Like Button */}
                <button
                ...
                >
                  ...
                  >
                    <ThumbsDown
                      className={cn("w-6 h-6", hasDisliked ? "fill-current" : "")}
                    />
                  </motion.div>
                  <span className={hasDisliked ? "font-medium" : ""}>
                    {formatCount(post.stats?.dislikesCount || 0)}
                  </span>
                </button>
                ...
                <div className="mt-4 pt-4 border-t border-buildops-border/20">
                  ...
                  >
                    ...
                    <div className="text-[15px] md:text-[16px] text-buildops-text/95 leading-relaxed font-sans whitespace-pre-wrap break-words">
                      {(() => {
                        const { excerpt } = getWordsExcerpt(post.body, 30);
                        return (
                          <span>
                            {excerpt}
          ...
          {/* Comments Section */}
          <section id="comments-section" className="pt-6 border-t border-buildops-border px-3 sm:px-4 md:px-5 lg:px-6 bg-buildops-bg">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-8 mb-6">
              <h2 className="text-xl font-bold text-buildops-text">
                Comments{" "}
                <span className="text-buildops-text-secondary font-normal text-base">
              ...
              {comments.length > 0 && (
                <div className="flex items-center gap-2 relative">
                  ...
                  <span className="text-buildops-text font-semibold text-sm">
                    Sort by
                  </span>
                  <select
                    value={commentSortBy}
                    onChange={(e) =>
                      setCommentSortBy(e.target.value as "top" | "newest")
                    }
              ...
              {comments.length === 0 ? (
                ...
                (() => {
                  const renderComment = (
                    ...
                    const parentComment =
                      depth > 0
                        ? comments.find((c: any) => c.id === comment.parentId)
                        : null;
                    const parentAuthor = parentComment
                      ? commentUsers[parentComment.authorId]
                      : null;
                    ...
                    return (
                      <div
                        ...
                        onTouchStart={() =>
                          handleCommentPressStart(comment.id, comment.authorId)
                        }
                      ...
                      >
                        <div className="flex gap-2 sm:gap-3">
                          <Link
                            ...
                            className="w-8 h-8 rounded-full border border-buildops-border bg-buildops-bg overflow-hidden flex items-center justify-center shrink-0 mt-1"
                          >
                            {author?.photoURL ? (
                              <img
                                src={author.photoURL}
                          ...
                          <div className="flex-1 min-w-0">
                              ...
                              <div className="text-sm text-buildops-text whitespace-pre-wrap break-words">
                                ...
                                comment.body.length > 300 ? (
                                  <>
                                    ...
                                    <button
                                      onClick={(e) =>
                                        toggleExpandedText(comment.id, e)
                                      }
                                      className="ml-2 text-buildops-blue text-xs font-medium hover:underline"
                                    >
                                      Read more
                                    </button>
                                  </>
                                    ...
                                    {expandedTextState[comment.id] &&
                                      ...
                                      comment.body.length > 300 && (
                                        <button
                                          ...
                                          className="ml-2 text-buildops-text-secondary text-xs font-medium hover:underline mt-1 block"
                                        >
                                          Show less
                                        </button>
                                      )}
                            ...
                            <div className="flex flex-wrap items-center gap-x-3 gap-y-2 mt-2">
                              <div className="flex items-center gap-1">
                                ...
                                >
                                  <ThumbsUp
                                    className={`w-3.5 h-3.5 ${userCommentVotes[comment.id] === 1 ? "fill-current" : ""}`}
                                  />
                                </button>
                                <span className="text-xs font-medium text-buildops-text-secondary">
                                  {formatCount(comment.score || 0)}
                                </span>
                                <button
                                  onClick={() =>
                                    handleVoteComment(comment.id, -1)
                                ...
                                >
                                  <ThumbsUp
                                    className={`w-3.5 h-3.5 rotate-180 ${userCommentVotes[comment.id] === -1 ? "fill-current" : ""}`}
                                  />
                                </button>
                              </div>
                              <button
                              ...
                              {comment.authorId === user?.uid &&
                                (() => {
                                  const notif = sentNotifications.find(
                                  ...
                                  <button
                                    onClick={async () => {
                                      try {
                                        ...
                                        if (comment.authorId) {
                                          batch.set(
                                            doc(db, "users", comment.authorId),
                                            {
                                              stats: {
                                                problemsSolved: increment(
                                                  isNowSolution ? 1 : -1,
                                                ),
                                                proofScore: increment(
                                                  isNowSolution ? 10 : -10,
                                                ),
                                              },
                                              updatedAt: serverTimestamp(),
                                ...
                                <div className="flex-1 flex flex-col min-w-0">
                                  ...
                                  <div className="flex flex-wrap justify-end gap-2 pr-1 pt-3">
                                    ...
                                    <button
                                      ...
                                      disabled={
                                        isSubmittingReply || !replyBody.trim()
                                      }
                        ...
                        {!hideReplies &&
                          replies.length > 0 &&
                          (() => {
                            const solutionCount = replies.filter((r) =>
                              isCommentInSolutionPath(r.id),
                            ).length;
                            const currentVisible =
                              expandedRepliesState[comment.id] !== undefined
                                ? expandedRepliesState[comment.id]
                                : solutionCount;
                            ...
                            return (
                              <div
                                className={
                                  depth < 4
                                    ? "mt-2 ml-4 sm:ml-5 pl-4 sm:pl-5 border-l-2 border-buildops-border/30"
                                    : "mt-2 pl-4 sm:pl-5 border-l-2 border-buildops-border/30"
                                }
                              >
                                ...
                                {hasMore && (
                                  <div className="mt-3 text-xs font-semibold text-buildops-blue">
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        setViewingRepliesFor(comment.id);
                                      }}
                                      className="hover:underline flex items-center gap-1.5 py-1 px-3 rounded-full bg-buildops-blue/10 transition-colors hover:bg-buildops-blue/20"
                                    >
                                      <MessagesSquare className="w-3.5 h-3.5" />
                                      {!isExpanded && currentVisible === 0
                  ...
                  return (
                    <>
                      ...
                      {visibleRootComments.length < rootComments.length &&
                        visibleRootComments.length >=
                          visibleTopCommentsCount && (
                          <div className="flex justify-center mt-6">
                            <button
                              onClick={() =>
                                setVisibleTopCommentsCount((prev) => prev + 5)
                              }
                              className="px-6 py-2 rounded-lg bg-buildops-card border border-buildops-border text-buildops-text font-medium text-sm hover:bg-white/5 transition-colors"
                            >
                              Load more comments
                            </button>
                          </div>
                      ...
                      {viewingRepliesFor &&
                        (() => {
                          ...
                          const threadReplies =
                            getAllDescendants(viewingRepliesFor);
                          const sortedThreadReplies = [...threadReplies].sort(
                            (a, b) => {
                              const aIsSolution = isCommentInSolutionPath(a.id);
                              const bIsSolution = isCommentInSolutionPath(b.id);
                              if (aIsSolution && !bIsSolution) return -1;
                            ...
                            <div className="fixed inset-0 z-[100] bg-buildops-bg overflow-y-auto flex flex-col">
                              ...
                              <div className="max-w-4xl mx-auto w-full px-4 py-6 flex-1 pb-32">
                                ...
                                <div className="space-y-4 ml-[15px] pl-4 sm:pl-6 border-l-2 border-buildops-border/30">
                                  {sortedThreadReplies.map((r) =>
                                    renderComment(r, 0, true),
                                  )}
                                </div>
                              </div>
                            </div>
      ...
      <aside className="hidden lg:block w-72 shrink-0 space-y-6 pt-2 lg:pt-[80px]">
        ...
        <div className="rounded-lg border border-buildops-border bg-buildops-card p-5 space-y-4">
          <div>
            <h3 className="text-sm font-bold text-buildops-text mb-3">
              Post Info
            </h3>
            <div className="space-y-2 text-sm text-buildops-text-secondary">
              {post.category && (
                <div className="flex justify-between">
                  <span>Category</span>
                  <span className="font-mono text-buildops-text lowercase">
                    {post.category}
                  </span>
                </div>
              )}
      ...
      {longPressedCommentId &&
        (() => {
          const commentToDelete = comments.find(
            (c) => c.id === longPressedCommentId,
          );
          const isAdminUser = userProfile?.role === "admin";
          const isCommentAuthor =
            commentToDelete && user?.uid === commentToDelete.authorId;
          const isAuthorized =
            commentToDelete && (isCommentAuthor || isAdminUser);
          ...
          return (
            ...
            >
              <div
                ...
                onClick={(e) => e.stopPropagation()}
              >
                <h3 className="text-lg font-bold text-buildops-text mb-1">
                  Delete comment?
                </h3>
          ...
          >
            <div className="p-6 border-b border-buildops-border flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-bold text-buildops-text">
                  Report Comment
                </h3>
              </div>
              <button
            ...
            <div className="p-6">
              ...
              <div className="space-y-1 max-h-[30vh] overflow-y-auto no-scrollbar">
                {[
                  ...
                  >
                    <span className="text-sm text-buildops-text font-medium">
                      {reason}
                    </span>
